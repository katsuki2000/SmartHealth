    ...
    var GlobalExceptionFilter = _classThis = /** @class */ (function () {
        ...
        GlobalExceptionFilter_1.prototype.catch = function (exception, host) {
            ...
            if (exception instanceof common_1.HttpException) {
                ...
                this.logger.error("Unhandled Exception: ".concat(exception.message), exception.stack, 'GlobalExceptionFilter');
            }
            var errorResponse = __assign({ statusCode: status, timestamp: new Date().toISOString(), path: request.url, method: request.method, message: message }, (details && { details: details }));
            // Log error for audit trail
            this.logger.warn("[".concat(request.method, "] ").concat(request.url, " - Status: ").concat(status, " - ").concat(message), 'GlobalExceptionFilter');
